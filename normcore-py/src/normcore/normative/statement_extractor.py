...
class StatementExtractor:
    ...
    def _contains_strong_normative_indicators(self, text: str) -> bool:
        ...
        """
        text_lower = text.lower()
        return any(re.search(pattern, text_lower) for pattern in self.NORMATIVE_INDICATORS)
    ...
    def _looks_like_protocol_sentence(self, sentence: str) -> bool:
        ...
        return s_lower.endswith("?") and not self._contains_normative_indicators(s_lower)
    ...
    def _strip_protocol_suffix(self, text: str) -> str:
        ...
        while prev != current and iteration < max_iterations:
            ...
            for pattern in self.PROTOCOL_SUFFIX_PATTERNS:
                ...
                current = re.sub(
                    anchored_pattern, "", current, flags=re.IGNORECASE | re.DOTALL
                ).strip()
    ...
    def _strip_greeting(self, text: str) -> str:
        ...
        if not self._contains_normative_indicators(cleaned):
            logger.debug(
                "StatementExtractor: No normative indicators found " "(likely protocol-only output)"
            )
        ...
        cleaned = re.sub(self.PROTOCOL_PREFIX_PATTERN, "", cleaned, flags=re.IGNORECASE).strip()
