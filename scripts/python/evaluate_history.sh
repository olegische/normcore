...
usage() {
  cat <<'EOF'
Usage:
  scripts/python/evaluate_history.sh <conversation.json> [--grounds <grounds.json>] [--agent-output <text>] [--log-level <LEVEL>] [-o|--output <judgment.json>]

Examples:
  scripts/python/evaluate_history.sh context/rollout.conversation.json
  scripts/python/evaluate_history.sh context/rollout.conversation.json --grounds context/grounds.json -o context/judgment.json
EOF
}
...
try:
    ...
    src_dir = Path.cwd() / "normcore-py" / "src"
