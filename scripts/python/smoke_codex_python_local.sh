...
if [[ ! -x "$ROOT_DIR/scripts/python/evaluate_history.sh" ]]; then
  echo "ERROR: missing executable script: $ROOT_DIR/scripts/python/evaluate_history.sh" >&2
  exit 2
fi
...
/bin/bash "$ROOT_DIR/scripts/python/evaluate_history.sh" \
  "$CONVERSATION_JSON" \
