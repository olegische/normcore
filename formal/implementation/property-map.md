...
| I-LINKS-REACHABILITY | invariant | In links mode, `ASSERTIVE` licensing is possible only when at least one strong factual `SUPPORTS` link resolves; with absent/weak factual grounding, effective assertive supports cannot be present. | `src/normcore/normative/license_deriver.py` | REQUIRED |
