...
class ModalityDetector:
    ...
    def __init__(self):
        ...
        self._conditional_re = [re.compile(ind, re.IGNORECASE) for ind in self.CONDITIONAL_FORM_INDICATORS]
        self._goal_conditional_re = [re.compile(ind, re.IGNORECASE) for ind in self.GOAL_CONDITIONAL_FORM_INDICATORS]
        self._personalization_conditional_re = [
            re.compile(ind, re.IGNORECASE) for ind in self.PERSONALIZATION_CONDITIONAL_FORM_INDICATORS
        ]
        self._descriptive_re = [re.compile(ind, re.IGNORECASE) for ind in self.DESCRIPTIVE_FORM_INDICATORS]
        self._normative_re = [re.compile(ind, re.IGNORECASE) for ind in self.NORMATIVE_FORM_INDICATORS]
        self._recommendation_re = [re.compile(ind, re.IGNORECASE) for ind in self.RECOMMENDATION_FORM_INDICATORS]
